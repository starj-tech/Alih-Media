...
export default function BerkasTimelineDialog({ berkas, open, onOpenChange }: Props) {
  ...
  const [ipToCheck, setIpToCheck] = useState<string | null>(null);
  ...
  const IpClickable = ({ ip }: { ip: string | undefined }) => {
    if (!ip || ip === '-') return <span className="font-medium">-</span>;
    return (
      <button
        onClick={() => handleCheckIp(ip)}
        className="font-medium text-primary hover:underline inline-flex items-center gap-1 cursor-pointer"
        title="Klik untuk cek lokasi IP"
      >
        {ip}
        ...
        <DialogContent className="max-w-lg">
          ...
          {berkas && (
            <div className="text-sm space-y-1 pb-3 border-b">
              ...
              <p><span className="text-muted-foreground">IP Server Pengajuan:</span> <IpClickable ip={berkas.ipAddress} /></p>
              <p><span className="text-muted-foreground">IP Perangkat Pengajuan:</span> <IpClickable ip={berkas.deviceIpAddress} /></p>
            </div>
          )}
